...
with sqlite3.connect("database (Miky).db") as db:
    ...
    class Tickets(commands.Cog):
        ...
        async def ticketsinformation(self, ctx):
            ...
            guild_tm = self.bot.get_guild(tumka_guild)
            guild_na = self.bot.get_guild(natame_guild)

            if perms_owner is not None or perms_dev is not None:
                if ctx.guild == guild_tm:
                    channel_tickets = self.bot.get_channel(990715925646962728)
                ...
                if ctx.guild == guild_na:
                    channel_tickets = self.bot.get_channel(1006836523523645480)
        ...
        async def on_dropdown(self, inter):
            ...
            if inter.guild == guild_tm:
                waiting_category = disnake.utils.get(inter.guild.categories, id=1016432288273285222)
            ...
            if inter.guild == guild_na:
                waiting_category = disnake.utils.get(inter.guild.categories, id=1007017659461796030)
                ...
                if inter.component.custom_id == "tickets_menu":
                    ...
                    if "tickets" in inter.values:
                        ...
                        await embed_msg.edit(content=f"<@&1007017421766393866>\nОбращение от {author_ticket.mention} | `{author_ticket.id}`", embed=embed, view=support_button)
                        await ticket_channel.set_permissions(author_ticket, read_messages=True, send_messages=True)
                        await msg.edit(content=f"Обращение создано. Для его заполнения перейдите в канал {ticket_channel.mention}.")
        ...
        async def on_button_click(self, inter):       
            ...
            if inter.guild == guild_tm:
                channel_logs = self.bot.get_channel(847050542652063754)

                processing_category = disnake.utils.get(inter.guild.categories, id=1016432311576825886)
                closed_category = disnake.utils.get(inter.guild.categories, id=1016432336583274566)

                curator_team_role = disnake.utils.get(inter.guild.roles, id=1005196724018610186)
                moderator_role = disnake.utils.get(inter.guild.roles, id=1005939612386070668)
                assistant_role = disnake.utils.get(inter.guild.roles, id=845693319971471380)
                ...
                if inter.component.custom_id == "take_ticket":
                    ...
                    await inter.message.edit(content=f"<@&1007017421766393866>\nОбращение от {author_ticket.mention} | `{author_ticket.id}`", embed=embed, view=support_button)
                    await ticket_channel.edit(category=processing_category)

                    await ticket_channel.set_permissions(moderator_role, overwrite=None)
                    await ticket_channel.set_permissions(assistant_role, overwrite=None)
                ...
                if inter.component.custom_id == "enforcement_ticket":
                    ...
                    if support_ticket == inter.author:
                        ...
                        await ticket_channel.set_permissions(support_ticket, overwrite=None)
                        await ticket_channel.set_permissions(curator_team_role, read_messages=True)
            ...
            if inter.guild == guild_na:
                channel_logs = self.bot.get_channel(1007019495954591814)

                processing_category = disnake.utils.get(inter.guild.categories, id=1017505105362309242)
                closed_category = disnake.utils.get(inter.guild.categories, id=1017505158009204886)

                security_role = disnake.utils.get(inter.guild.roles, id=1006622510822989996)
                tickets_role = disnake.utils.get(inter.guild.roles, id=1007017421766393866)
                ...
                if inter.component.custom_id == "take_ticket":
                    ...
                    embed_msg_db = cursor.execute(f"""SELECT embed_message_id FROM tickets WHERE channel_id == {ticket_channel.id} AND guild_id == {inter.guild.id}""").fetchone()[0]
                    embed_msg = self.bot.get_message(embed_msg_db)
                    ...
                    await ticket_channel.set_permissions(tickets_role, overwrite=None)
                    await ticket_channel.set_permissions(support_ticket, read_messages=True, send_messages=True)
        ...
        async def ticketschannelclear(self, ctx):
            ...
            if ctx.guild == guild_tm:
                category_closed = disnake.utils.get(ctx.guild.categories, id=1016432336583274566)
            if ctx.guild == guild_na:
                category_closed = disnake.utils.get(ctx.guild.categories, id=1017505158009204886)
